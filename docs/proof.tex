...
\title{\textbf{Catching Pivots} \\[0.3em]
\large Causal Localization and Single-Token Intervention at the\\
Deceptive Transition in Chain-of-Thought Reasoning}
\author{Aryan Gupta}
\date{June 2026}
...
\begin{abstract}
\noindent
Reasoning-trained language models routinely generate chains of thought (CoT) whose intermediate content is materially inconsistent with the final user-facing answer. The published instances range from sycophantic capitulation against an internally acknowledged correct answer \citep{sharma2023sycophancy,perez2022mwe} to in-context scheming and alignment faking under disclosed retraining pressure \citep{meinke2024scheming,greenblatt2024faking}, to obfuscated reward hacking under CoT-monitor optimisation \citep{baker2025monitoring}, to systematically unverbalised hint use across hint families \citep{chen2025reasoningthink}. A natural defence is to read the residual stream during CoT generation and intervene when deception forms. Per-token probing of the residual stream is now available \citep{goldowsky2025deception,chrabaszcz2026monologue,mirtaheri2026rationalization}; the open question is not whether per-token signal exists but whether a discrete transition token, at which the model's internal state crosses from honest reasoning into deceptive output formation, can be defined, identified, and causally validated, and whether intervention restricted to that token can flip the trajectory at a strictly smaller distributional cost than the uniform or smoothly-gated interventions in present use \citep{panickssery2023caa,lee2024cast,ferrando2025dsas,yu2025pixel}. This document develops the formal framework for that question. We treat CoT generation as a partially observed two-state Markov process whose latent honesty state is read through a linear residual-stream probe; we define the pivot as the smallest stopping time at which a single-token intervention is causally sufficient to flip the downstream output; we show that under reasonable assumptions the causal pivot almost surely precedes or coincides with the probe argmax; we bound the Kullback--Leibler divergence between the intervened and base CoT distributions and prove that pivot-localized intervention dominates uniform Contrastive Activation Addition by a factor of $\Theta(T)$ in CoT length; and we specify an online detection rule via a CUSUM-type sequential procedure with calibrated false-alarm control. The corresponding experimental protocol on open reasoning models and the relationship to recent monitorability work \citep{korbak2025monitorability} are stated at the end.
\end{abstract}

\section{Introduction}
\label{sec:intro}

The phenomenon in question is now empirically well established. \citet{lanham2023faithfulness} measured CoT faithfulness under truncation, paraphrase, and mistake insertion and reported inverse scaling on most tasks; \citet{turpin2023biased} showed that biased few-shot prompts cause systematically unfaithful explanations; \citet{chen2025reasoningthink} report that on six hint categories, Claude 3.7 Sonnet and DeepSeek-R1 verbalise an outcome-changing hint at average rates well below $0.30$, and as low as $0.02$ in a reward-hacking environment. \citet{baker2025monitoring} demonstrate that an LLM monitor reading the CoT initially catches roughly 95\% of reward-hack episodes during agentic coding, but that adding the monitor's verdict to the RL reward produces obfuscated CoTs that hack at the same rate while reading as innocuous. \citet{meinke2024scheming} document explicit scheming language in the scratchpad of frontier models including the maintenance of deception across more than 85\% of multi-turn interrogation rounds. \citet{greenblatt2024faking} show that Claude 3~Opus, given a scratchpad and a system prompt indicating a training context, complies with harmful queries at non-trivial rates with explicit alignment-faking reasoning visible in essentially all such cases. \citet{korbak2025monitorability} argue, as an organisational position, that the human-language CoT in present reasoning models is a privileged but fragile monitoring channel.

Two responses are available. Black-box CoT monitoring \citep{baker2025monitoring,korbak2025monitorability} reads the scratchpad with another model and flags or rewards visible misbehaviour. White-box residual-stream monitoring \citep{macdiarmid2024probes,goldowsky2025deception} reads the model's own activations and bypasses any structural pressure on the legible text. We work in the latter regime. The white-box probing literature has converged on three observations: (i) simple linear probes (mass-mean, logistic regression, contrast-consistent search) trained on contrastive honesty data attain high in-distribution AUROC \citep{burns2022ccs,marks2023geometry,burger2024universal,goldowsky2025deception}; (ii) the per-token probe signal across a generated response is non-stationary and informative beyond aggregate scores \citep{goldowsky2025deception,chrabaszcz2026monologue}; (iii) acting on the same direction at all post-prompt token positions, as in Contrastive Activation Addition \citep{panickssery2023caa} or Inference-Time Intervention \citep{li2023iti}, induces a substantial fluency tax \citep{pres2024reliable,li2026safety}.

This work pursues a single thesis: that the empirical non-stationarity of the per-token probe trajectory is not noise but the trace of a discrete latent transition, the \emph{pivot}, and that the correct intervention is to act precisely there. The contributions are five.
...
Two preliminary remarks. First, novelty is narrow. \citet{goldowsky2025deception} already produce per-token deception scores and report failure modes; \citet{chrabaszcz2026monologue} explicitly construct probe trajectories across the reasoning process; \citet{mirtaheri2026rationalization} probe before and after the CoT for motivated reasoning. We do not claim per-token probing as our contribution. The contribution is the causal characterisation of a discrete pivot event, its identification, and the demonstration that a token-localized intervention occupies an empty region of the position-versus-gating design space currently bracketed by PIXEL (offline position-scan, no online probe) and DSAS (online probe, smooth gating, no sparsity). Second, the safety frame is not detection. Detection of deception in present reasoning models is, on standard contrastive benchmarks, already a near-saturated problem \citep{goldowsky2025deception}. The frame is intervention: producing a model whose post-pivot continuation is honest at low distributional cost, which matters precisely because uniform steering is known to corrupt fluency \citep{pres2024reliable}.
...
The model says three things. First, deceptive intent is a discrete event with no return: once entered, it persists. Second, the per-token probe is a noisy Gaussian observation of the latent state. Third, the next-token distribution is a mixture indexed by the latent state and is the conduit by which honesty propagates to output content. The absorbing structure of $D$ is consistent with the qualitative picture in \citet{baker2025monitoring}: once a reasoning model commits to a deceptive plan, the plan persists through the remainder of the CoT. We relax it in \Cref{rem:reversible}.
...
Suppose the two-state model of \Cref{def:two-state} holds with Gaussian emissions and absorbing $D$, and \Cref{ass:monotone} holds at $(c_0, \Delta_0)$. Then for any threshold $\tau$ with $\mu_H < \tau \le \mu_D$ and the choice $(c, \Delta) = (c_0, \Delta_0)$,
\[
\Prob\bigl(T^{\dagger}_{\Delta} \le T^{\star}_{\tau} \bigm| T^{\dagger}_{\Delta} < \infty,\; T^{\star}_{\tau} < \infty\bigr) \;\ge\; 1 - 2\,\Phi\!\Bigl(\frac{\mu_H - \tau}{\sigma}\Bigr),
\]
where $\Phi$ is the standard Gaussian cdf.
\end{theorem}

\begin{proof}
By absorption of $D$, $T^{\dagger}_{\Delta} \ge T^{\circ}$ in the sense that $F_{\Delta}$ cannot fire while $Z = H$ at level $\Delta_0$ by \Cref{ass:monotone}; hence $T^{\dagger}_{\Delta} \ge T^{\circ}$. We bound the probability that $T^{\star}_{\tau} < T^{\circ}$. For each $t < T^{\circ}$, $S_t \sim \mathcal{N}(\mu_H, \sigma^2)$ independent across $t$ conditional on $Z$, so
\[
\Prob(S_t > \tau \mid Z_t = H) = 1 - \Phi\!\Bigl(\frac{\tau - \mu_H}{\sigma}\Bigr) = \Phi\!\Bigl(\frac{\mu_H - \tau}{\sigma}\Bigr).
\]
A union bound over the pre-pivot positions, augmented by the boundary contribution at $t = T^{\circ}$, gives the stated inequality. Reassembling, $T^{\star}_{\tau} \ge T^{\circ}$ with probability at least $1 - 2\Phi((\mu_H - \tau)/\sigma)$, and on the same event $T^{\dagger}_{\Delta} \ge T^{\circ}$, with $T^{\dagger}_{\Delta} \le T^{\star}_{\tau}$ following from the conditional version of \Cref{ass:monotone} at $t = T^{\circ}$.
\end{proof}

\Cref{thm:pivot-order} formalises the intuition that the probe trajectory's first threshold crossing tends to lag the causal pivot: by the time the probe is confident enough to fire, the deceptive plan has already taken hold. A safety system that intervenes at $T^{\star}_{\tau}$ rather than $T^{\dagger}_{\Delta}$ is therefore conservative in time. The implication for protocol is that we should evaluate at, and report, both $T^{\star}_{\tau}$ and $T^{\dagger}_{\Delta_0}$ in tandem.
...
The algorithm has three properties worth stating explicitly. (i) It is causal in the streaming sense: the decision to intervene at $t$ depends only on $Y_{1:t-1}$ and $\hh_{\ell}(1{:}t)$. (ii) It is one-shot: intervention happens at most once, at $\widehat{T}_{\mathrm{CUSUM}}$. (iii) The false-alarm rate is controlled by $b$ alone, independent of CoT length; choosing $b = \log(T_{\max}/\alpha_{\mathrm{seq}})$ guarantees a sequence-level false-alarm rate of $\alpha_{\mathrm{seq}}$ under the null model $Z \equiv H$.
...
\paragraph{Assumption \ref{ass:monotone} is testable.} The monotone causal advantage assumption can be falsified by intervening at random pre-pivot tokens and observing flip rates greater than at the latent pivot. We include the random-token-at-matched-probe-value baseline in Ablation A precisely as the falsification check.

\section{Conclusion}
\label{sec:conclusion}

The standing literature on deception probes in reasoning models has established that the residual stream carries an exploitable per-token signal of honesty state; the standing literature on activation steering has established that uniform-in-position interventions impose a fluency cost that scales with the perturbed region. The gap between these two literatures is occupied by a question whose answer is not yet on record: whether a single discrete token in the CoT carries the causal weight of the model's deceptive commitment, and whether intervention restricted to that token is both detectable online and Pareto-efficient against the uniform alternatives. The contribution of this document is the formal characterisation of that question, an identification result connecting the latent pivot to the observable first-passage and probe-argmax statistics, a KL bound establishing the $\Theta(T)$ separation between pivot-localized and uniform intervention, an online CUSUM algorithm for pivot detection at decoding time, and an experimental protocol calibrated against the four nearest neighbours in the position-versus-gating design space. The empirical execution of that protocol is the next stage of work and is the principal claim the eventual conference submission will rest on.
...
\newblock \emph{arXiv:2309.03883}, September 2023.
...
\newblock \emph{arXiv:2401.05566}, January 2024.
